...
public class LeaveRequestController {
    ...
    // ── WS helper ────────────────────────────────────────────────────────────
    // FIX F15: every template.convert* call is wrapped in a non-blocking
    // fire-and-forget helper. Previously a broker failure propagated as
    // MessageDeliveryException up to the HTTP handler and returned HTTP 500
    // even though the DB write had already succeeded. That caused the client
    // to retry, creating duplicate leave records.
...
//            String email = jwt.getClaim("email");  // "employee1@example.com"
     ...
     */
    @PostMapping("/apply")
    @PreAuthorize("hasAnyRole('GENERAL','HR', 'REPORTING_MANAGER') and hasAuthority('EDIT_TIMESHEET') and @permissionService.isOwner(authentication, #request.employeeId)")
...
//            template.convertAndSend("/topic/data-updated", "updated");
    ...
    public ResponseEntity<ApiResponse<LeaveRequest>> approveRequest(@Valid @RequestBody ApprovalRequestDTO approvalRequest) {
        try {
            ...
            LeaveRequest approvedRequest = leaveRequestService.approveRequest(approvalRequest, employee.getEmployeeId());
    ...
    @PreAuthorize("@permissionService.isOwner(authentication, #employeeId) or @permissionService.isManager(authentication, #employeeId) or hasAnyRole('HR', 'REPORTING_MANAGER')")
